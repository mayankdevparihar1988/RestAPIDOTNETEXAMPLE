...
namespace cleancodeprojectwebapi.Controllers
{
    ...
    public class RestaurantsController : ControllerBase
    {
        ...
        {
            var restaurant = await _mediator.Send(new GetRestaurantByIdQuery()
            {
                Id = id
            });
        ...
        [ProducesResponseType(StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        public async Task<ActionResult<RestaurantDto>> DeleteById([FromRoute] int id)
        {
            var restaurant = await _mediator.Send(new DeleteRestaurantCommand()
            {
                Id = id
            });
